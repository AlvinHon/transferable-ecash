...
use crate::proof::{check_ppe_proof_xbxb_t, create_ppe_proof_xbxb, PPEProof, CRS};
...
impl<E: Pairing> VerifyKey<E> {
    ...
    pub fn generate_proof<R: RngCore>(
        ...
        create_ppe_proof_xbxb(rng, crs, sig.z, self.gz, sig.r, self.gr)
    }
    ...
    /// ```
    pub fn check_proof(&self, crs: &CRS<E>, cpf: &PPEProof<E>, m: &[E::G1Affine]) -> bool {
        let target = m
            .iter()
            .zip(&self.pk)
            .map(|(m, pk)| E::pairing(*m, *pk))
            .fold(PairingOutput::zero(), |acc, m| acc + m);
        check_ppe_proof_xbxb_t(crs, cpf, self.gz, self.gr, target)
    }
}

#[cfg(test)]
mod tests {
    use crate::{lhsps::setup, proof::CRS};
    ...
    fn test_derived_signature() {
        ...
        let pf = pk.generate_proof(rng, &crs, &sig_d);
